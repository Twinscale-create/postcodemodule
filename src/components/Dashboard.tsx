...
export default function Dashboard({ matches, kansen, klantGebieden }: DashboardProps) {
  ...
  const [klantSort,     setKlantSort]     = useState<'klanten' | 'pen' | 'postcode'>('klanten')
  ...
  function handleKlantSort(key: typeof klantSort) {
  ...
  const topKansen = kansen
    ...
    .sort((a, b) => {
      ...
      if (kansenSort === 'postcode') diff = Number(a.postcode) - Number(b.postcode)
        ...
        const penA = (klantCountByPostcode.get(a.postcode) ?? 0) / (a.inwoners || PC4_INWONERS_GEM)
        const penB = (klantCountByPostcode.get(b.postcode) ?? 0) / (b.inwoners || PC4_INWONERS_GEM)
        diff = penA - penB
      } else {
  ...
  const sortedKlanten = [...filteredKlanten].sort((a, b) => {
    ...
    else if (klantSort === 'pen') {
      diff = (a.klant_count / (a.inwoners || PC4_INWONERS_GEM)) - (b.klant_count / (b.inwoners || PC4_INWONERS_GEM))
    } else {
      diff = a.klant_count - b.klant_count
    }
    return klantSortDir === 'desc' ? -diff : diff
  })
  const maxKlantCount = Math.max(...sortedKlanten.map(x => x.klant_count), 1)
  const maxKlantPen   = Math.max(...sortedKlanten.map(x => x.klant_count / (x.inwoners || PC4_INWONERS_GEM)), 0.001)
  ...
  const PC4_AVG_INWONERS = 5500
  const CONV_RATE        = 0.002
  const AVG_ORDER_VALUE  = 150
  const pot_bereik  = top10.length * PC4_AVG_INWONERS
  const pot_score   = top10.length ? Math.round(top10.reduce((s, k) => s + k.score, 0) / top10.length) : 0
  const pot_klanten = Math.round(pot_bereik * (pot_score / 100) * CONV_RATE)
  const pot_omzet   = pot_klanten * AVG_ORDER_VALUE
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden', background: themeGradient ?? 'var(--bg)', transition: 'background 0.5s ease', fontFamily: 'var(--font)' }}>
        ...
        <div style={{
          ...
          <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '1px' }}>
            {topKansen.map((k, i) => {
              ...
              const pen  = Math.round((klantCountByPostcode.get(k.postcode) ?? 0) / (k.inwoners || PC4_INWONERS_GEM) * 100)
              return (
                <button key={k.postcode} onClick={() => handleSelectKans(k)} style={{
                  ...
                  <span style={{ fontSize: '9px', fontWeight: 700, color: S.t4, width: '14px', textAlign: 'right', flexShrink: 0, fontFamily: 'var(--font-mono)' }}>{i+1}</span>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1px', width: '60px', flexShrink: 0 }}>
                    <span style={{ fontSize: '13px', fontWeight: 700, color: isSelected ? kc : S.t1, lineHeight: 1, fontFamily: 'var(--font-mono)', letterSpacing: '-0.01em' }}>{k.postcode}</span>
                    {stad && <span style={{ fontSize: '9px', color: S.t4, lineHeight: 1, fontFamily: 'var(--font)' }}>{stad}</span>}
                  </div>
                  {pen > 0 && (
            ...
            <>
              <div style={{ display: 'flex', alignItems: 'center', gap: '7px', marginBottom: '10px', flexShrink: 0 }}>
                ...
                <button onClick={() => { setSelectedKans(null); setSelectedCbs(null) }}
                  style={{ marginLeft: 'auto', fontSize: '14px', color: S.t4, background: 'none', border: 'none', cursor: 'pointer', lineHeight: 1, padding: '2px 4px', borderRadius: '4px', transition: 'color 0.12s' }}>✕</button>
              </div>
              <p className="ts-label" style={{ marginBottom: '8px', flexShrink: 0 }}>CBS-profiel match</p>
              {/* Header: jouw klanten vs dit gebied */}
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px', flexShrink: 0 }}>
                <span style={{ width: '58px', flexShrink: 0 }} />
                <span style={{ fontSize: '9px', fontWeight: 700, color: S.t4, width: '52px', flexShrink: 0, textAlign: 'center', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Klanten</span>
                <span style={{ fontSize: '9px', fontWeight: 700, color: S.t4, width: '52px', flexShrink: 0, textAlign: 'center', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Gebied</span>
                <span style={{ fontSize: '9px', fontWeight: 700, color: S.t4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Match</span>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '7px', overflowY: 'auto' }}>
                {(() => {
                  const rows = [
                    { Icon: Users,     label: 'Leeftijd', klant: profiel.leeftijd,    area: selectedCbs.cbs_leeftijd,   max: 70,    fmt: (v: number) => `${v}jr`,                                                                     color: '#C4A840' },
                    { Icon: TrendingUp, label: 'Vrouwen',  klant: profiel.pctVrouw,    area: selectedCbs.pct_vrouw,      max: 100,   fmt: (v: number) => `${v}%`,                                                                      color: '#C87090' },
                    { Icon: Home,       label: 'Koop',     klant: profiel.pctKoop,     area: selectedCbs.pct_koop,       max: 100,   fmt: (v: number) => `${v}%`,                                                                      color: '#9B72CF' },
                    { Icon: Building2,  label: 'Markt',    klant: profiel.avgInwoners, area: selectedKans?.inwoners ?? 0, max: 20000, fmt: (v: number) => v >= 1000 ? `${(v/1000).toFixed(1)}K` : `${v}`, color: '#5BA4CF' },
                  ]
                  return rows.map(({ Icon, label, klant, area, max, fmt, color }) => {
                    const diff = Math.abs(klant - area) / max
                    const matchColor = diff < 0.1 ? '#8FB431' : diff < 0.25 ? '#C4A840' : '#C85040'
                    const matchLabel = diff < 0.1 ? '✓ Goed' : diff < 0.25 ? '~ Ok' : '✗ Ver'
                    return (
                      <div key={label} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <Icon size={10} color={S.t4} style={{ flexShrink: 0 }} />
                        <span style={{ fontSize: '10px', color: S.t3, width: '50px', flexShrink: 0 }}>{label}</span>
                        {/* Klant waarde */}
                        <div style={{ width: '52px', flexShrink: 0, textAlign: 'center' }}>
                          <span style={{ fontSize: '10px', fontWeight: 700, color: S.t2, fontFamily: 'var(--font-mono)' }}>{fmt(Math.round(klant))}</span>
                        </div>
                        {/* Gebied waarde */}
                        <div style={{ width: '52px', flexShrink: 0, textAlign: 'center', background: `${color}10`, borderRadius: '4px', padding: '1px 0' }}>
                          <span style={{ fontSize: '10px', fontWeight: 700, color, fontFamily: 'var(--font-mono)' }}>{fmt(typeof area === 'number' ? Math.round(area * 10) / 10 : area)}</span>
                        </div>
                        {/* Match indicator */}
                        <span style={{ fontSize: '9px', fontWeight: 700, color: matchColor, background: `${matchColor}14`, border: `1px solid ${matchColor}30`, borderRadius: '4px', padding: '1px 6px', whiteSpace: 'nowrap' }}>{matchLabel}</span>
                      </div>
                    )
                  })
                })()}
          ...
          {selectedKlant && !loadingKlantCbs && selectedKlantCbs && (
            <>
              <div style={{ display: 'flex', alignItems: 'center', gap: '7px', marginBottom: '10px', flexShrink: 0 }}>
                ...
                <div style={{ background: S.bg3, borderRadius: 'var(--r-sm)', padding: '3px 8px', fontSize: '11px', fontWeight: 700, color: S.t3, border: `1px solid ${S.bdr}`, fontFamily: 'var(--font-mono)' }}>
                  {selectedKlant.klant_count} klanten · {((selectedKlant.klant_count / (selectedKlant.inwoners || 5500)) * 100).toFixed(1)}% pen.
                </div>
              ...
              <p className="ts-label" style={{ marginBottom: '8px', flexShrink: 0 }}>CBS-profiel klantgebied</p>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px', flexShrink: 0 }}>
                <span style={{ width: '58px', flexShrink: 0 }} />
                <span style={{ fontSize: '9px', fontWeight: 700, color: S.t4, width: '52px', flexShrink: 0, textAlign: 'center', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Gem.</span>
                <span style={{ fontSize: '9px', fontWeight: 700, color: S.t4, width: '52px', flexShrink: 0, textAlign: 'center', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Gebied</span>
                <span style={{ fontSize: '9px', fontWeight: 700, color: S.t4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Match</span>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '7px', overflowY: 'auto' }}>
                {(() => {
                  const rows = [
                    { Icon: Users,     label: 'Leeftijd', klant: profiel.leeftijd,    area: selectedKlantCbs.cbs_leeftijd,       max: 70,    fmt: (v: number) => `${v}jr`,                                                                     color: '#C4A840' },
                    { Icon: TrendingUp, label: 'Vrouwen',  klant: profiel.pctVrouw,    area: selectedKlantCbs.pct_vrouw,          max: 100,   fmt: (v: number) => `${v}%`,                                                                      color: '#C87090' },
                    { Icon: Home,       label: 'Koop',     klant: profiel.pctKoop,     area: selectedKlantCbs.pct_koop,           max: 100,   fmt: (v: number) => `${v}%`,                                                                      color: '#9B72CF' },
                    { Icon: Building2,  label: 'Markt',    klant: profiel.avgInwoners, area: selectedKlant?.inwoners ?? 0,        max: 20000, fmt: (v: number) => v >= 1000 ? `${(v/1000).toFixed(1)}K` : `${v}`, color: '#5BA4CF' },
                  ]
                  return rows.map(({ Icon, label, klant, area, max, fmt, color }) => {
                    const diff = Math.abs(klant - area) / max
                    const matchColor = diff < 0.1 ? '#8FB431' : diff < 0.25 ? '#C4A840' : '#C85040'
              ...
              {([
                ...
                return (
                  <button key={id} onClick={() => setViewTab(id)} style={{
                    ...
                    padding: '0 18px', fontSize: '11px', fontWeight: 700,
                    ...
                    <Icon size={10} color={isActive ? accent : 'var(--t-4)'} />
                    {label}
                    {id === 'postcodes' && (
                      <span style={{ fontSize: '9px', fontWeight: 700, background: 'var(--accent-bg)', color: 'var(--accent)', borderRadius: '3px', padding: '1px 5px', marginLeft: '2px', fontFamily: 'var(--font-mono)' }}>
                        {topKansen.length}
                      </span>
                    )}
                    {id === 'klanten' && klantGebieden.length > 0 && (
                      <span style={{ fontSize: '9px', fontWeight: 700, background: 'rgba(103,191,218,0.10)', color: 'var(--accent-2)', borderRadius: '3px', padding: '1px 5px', marginLeft: '2px', fontFamily: 'var(--font-mono)' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        {/* ══ POSTCODES TAB ══ */}
        {viewTab === 'postcodes' && (
          <div style={{ flex: 1, overflowY: 'auto', background: 'transparent', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: '10px' }}>

            {/* Koptekst */}
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexShrink: 0 }}>
              <div style={{ width: '5px', height: '5px', borderRadius: '50%', background: 'var(--accent)', opacity: 0.85, flexShrink: 0 }} />
              <span style={{ fontSize: '11px', fontWeight: 700, color: S.t1, letterSpacing: '0.04em', textTransform: 'uppercase' }}>Kansen · CBS-profiel match</span>
              <span style={{ fontSize: '10px', color: S.t4, fontStyle: 'italic' }}>leeftijd · %vrouw · %koop · inwoners</span>
              <span style={{ fontSize: '9px', fontWeight: 700, background: 'var(--accent-bg)', color: 'var(--accent)', borderRadius: '3px', padding: '1px 6px', fontFamily: 'var(--font-mono)', marginLeft: 'auto' }}>{topKansen.length} kansen</span>
            </div>

            {/* Table header with sort */}
            <div style={{ display: 'grid', gridTemplateColumns: '28px 110px 120px 90px 1fr', gap: '0', alignItems: 'center', padding: '0 14px', flexShrink: 0 }}>
              {([
                { key: null,        label: '#' },
                ...
                <span key={h.label}
                  ...
                  style={{
                    fontSize: '8px', fontWeight: 700, color: h.key && kansenSort === h.key ? 'var(--accent)' : S.t4,
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
              {topKansen.map((k, i) => {
                ...
                const penPct       = Math.round(klantCount / (k.inwoners || PC4_INWONERS_GEM) * 100)
                ...
                return (
                  <div key={k.postcode}
                    ...
                    style={{
                      display: 'grid', gridTemplateColumns: '28px 110px 120px 90px 1fr',
                    ...
                    {/* Rank */}
                    <span style={{ fontSize: '10px', fontWeight: 700, color: S.t4, fontFamily: 'var(--font-mono)' }}>{i + 1}</span>

                    {/* Postcode + stad */}
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', padding: '0 4px' }}>
                      <span style={{ fontSize: '14px', fontWeight: 700, color: isSelected ? kc : S.t1, fontFamily: 'var(--font-mono)', letterSpacing: '-0.01em', lineHeight: 1 }}>{k.postcode}</span>
                      {stad && <span style={{ fontSize: '9px', color: S.t4, lineHeight: 1 }}>{stad}</span>}
                    </div>

                    {/* Score bar */}
                    <div style={{ padding: '0 4px', display: 'flex', alignItems: 'center', gap: '7px' }}>
                      <div style={{ flex: 1, height: '5px', background: S.bdr2, borderRadius: '3px', overflow: 'hidden' }}>
                        <div style={{ height: '100%', width: `${k.score}%`, background: `linear-gradient(90deg, ${kc}80, ${kc})`, borderRadius: '3px', boxShadow: isSelected ? `0 0 8px ${kc}` : 'none' }} />
                      </div>
                    ...
                    <div style={{ padding: '0 4px', display: 'flex', flexDirection: 'column', gap: '3px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                        <div style={{ width: '36px', height: '3px', background: S.bdr2, borderRadius: '2px', overflow: 'hidden', flexShrink: 0 }}>
                          <div style={{ height: '100%', width: `${penPct}%`, background: penPct >= 50 ? '#D44030' : penPct >= 20 ? '#C48830' : '#B8A440', borderRadius: '2px' }} />
                        </div>
                        <span style={{ fontSize: '11px', fontWeight: 700, color: penPct >= 50 ? '#D44030' : penPct >= 20 ? '#C48830' : S.t3, fontFamily: 'var(--font-mono)' }}>
                          {penPct}%
                        </span>
                      </div>
                      {klantCount > 0 && (
                        <span style={{ fontSize: '9px', color: S.t4 }}>{klantCount} klanten</span>
                      )}
                    </div>
                      ...
                      {nearestKlant ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '5px', flexWrap: 'wrap' }}>
                            <span style={{ fontSize: '12px', fontWeight: 700, color: 'var(--accent-2)', fontFamily: 'var(--font-mono)' }}>
                              {nearestKlant.postcode}
                            </span>
                            {nkStad && (
                              <span style={{ fontSize: '10px', color: S.t2, fontWeight: 500 }}>{nkStad}</span>
                            )}
                          </div>
                          <span style={{ fontSize: '9px', color: S.t4 }}>
                            {nearestKlant.klant_count} klanten · {Math.round(nearestKlant.klant_count / (nearestKlant.inwoners || PC4_INWONERS_GEM) * 100)}% pen.
                          </span>
                        </div>
                      ) : <span style={{ color: S.t4, fontSize: '10px' }}>—</span>}
                    </div>
                  </div>
                )
              })}
        ...
        {viewTab === 'klanten' && (
          <div style={{ flex: 1, overflowY: 'auto', background: 'transparent', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
            ...
            {klantGebieden.length > 0 && (() => {
              ...
              const avgPen = (klantGebieden.reduce((s, k) => s + k.klant_count / ((k.inwoners || PC4_INWONERS_GEM)), 0) / klantGebieden.length * 100).toFixed(1)
              return (
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px', flexShrink: 0 }}>
                  {[
                    { label: 'Totaal klanten',  value: String(total),                color: '#52C4E8', glow: 'rgba(82,196,232,0.20)' },
                    { label: 'Klantgebieden',   value: String(klantGebieden.length), color: '#4AB0D8', glow: 'rgba(74,176,216,0.18)' },
                    { label: 'Gem. penetratie', value: `${avgPen}%`,                 color: '#8FD44C', glow: 'rgba(143,212,76,0.18)'  },
                  ].map(s => (
                    <div key={s.label} style={{
                      ...
                      textAlign: 'center',
                      boxShadow: `0 8px 32px rgba(0,0,60,0.40), inset 0 1px 0 rgba(255,255,255,0.06)`,
                    }}>
                      <div style={{ fontSize: '38px', lineHeight: 1, color: s.color, fontFamily: 'var(--font-dotmatrix)', letterSpacing: '0.04em', textShadow: `0 0 18px ${s.glow}` }}>{s.value}</div>
                      <div style={{ fontSize: '8px', color: 'rgba(232,245,242,0.40)', marginTop: '5px', textTransform: 'uppercase', letterSpacing: '0.14em', fontFamily: 'var(--font)' }}>{s.label}</div>
                    </div>
                  ))}
                </div>
              )
            ...
            {/* Table header with sort */}
            <div style={{ display: 'grid', gridTemplateColumns: '28px 100px 90px 90px 80px 80px 80px 80px', gap: '0', alignItems: 'center', padding: '0 14px', flexShrink: 0 }}>
              {([
                { key: null,       label: '#' },
                { key: 'postcode', label: 'Postcode' },
                { key: 'klanten',  label: 'Klanten' },
                { key: 'pen',      label: 'Penetratie' },
                { key: null,       label: 'Leeftijd' },
                { key: null,       label: '%Vrouw' },
                { key: null,       label: '%Koop' },
                { key: null,       label: 'Inwoners' },
              ] as { key: 'klanten'|'pen'|'postcode'|null; label: string }[]).map(h => (
                <span key={h.label}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
              {sortedKlanten.map((k, i) => {
                const stad   = getStad(k.postcode)
                const inw    = (k.inwoners || PC4_INWONERS_GEM)
                const pen    = k.klant_count / inw * 100
                const penStr = pen.toFixed(1) + '%'
                const max    = klantSort === 'pen' ? maxKlantPen * PC4_INWONERS_GEM : maxKlantCount
                ...
                return (
                  <div key={k.postcode}
                    ...
                    style={{
                      display: 'grid', gridTemplateColumns: '28px 100px 90px 90px 80px 80px 80px 80px',
                      alignItems: 'center', gap: '0',
                    ...
                    <span style={{ fontSize: '10px', fontWeight: 700, color: S.t4, fontFamily: 'var(--font-mono)' }}>{i + 1}</span>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', padding: '0 4px' }}>
                      <span style={{ fontSize: '13px', fontWeight: 700, color: 'var(--accent-2)', fontFamily: 'var(--font-mono)', lineHeight: 1 }}>{k.postcode}</span>
                      {stad && <span style={{ fontSize: '9px', color: S.t4, lineHeight: 1 }}>{stad}</span>}
                    </div>
                    <div style={{ padding: '0 4px', display: 'flex', alignItems: 'center', gap: '5px' }}>
                      <span style={{ fontSize: '13px', fontWeight: 700, color: bc, fontFamily: 'var(--font-mono)' }}>{k.klant_count}</span>
                    </div>
                    <div style={{ padding: '0 4px' }}>
                      <span style={{ fontSize: '12px', fontWeight: 700, color: penColor, fontFamily: 'var(--font-mono)' }}>{penStr}</span>
                    </div>
                    {/* CBS columns — gemiddelde over matches (klik voor detail) */}
                    {[
                      `${profiel.leeftijd}jr`,
                      `${profiel.pctVrouw}%`,
                      `${profiel.pctKoop}%`,
                      profiel.avgInwoners >= 1000 ? `${(profiel.avgInwoners/1000).toFixed(1)}K` : `${profiel.avgInwoners}`,
                    ].map((val, ci) => (
                      <span key={ci} style={{ padding: '0 4px', fontSize: '11px', color: S.t3, fontFamily: 'var(--font-mono)' }}>{val}</span>
                    ))}
                  </div>
                )
              })}
            ...
            <div style={{ fontSize: '9px', color: S.t4, paddingTop: '4px', lineHeight: 1.6, letterSpacing: '0.02em' }}>
              Penetratie = klanten / CBS inwoners per PC4 · Klik op rij voor CBS-profiel detail
            </div>
          </div>
        )}

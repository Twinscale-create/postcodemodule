...
def nightly_pipeline():
    ...
    Stappen:
    1. Kassadata ophalen (CRM/API)
    2. CBS demografische data ophalen (meest recente dataset)
    3. Klantprofiel bouwen uit kassadata
    4. Matching engine — top matches berekenen
    5. Kansen kaart — scores voor ALLE NL postcodes (elke week)
    6. Resultaten opslaan in Supabase + lokale cache
    """
    print(f"\n🌙 Nachtelijke pipeline gestart: {datetime.now()}")
    date_to   = datetime.now().strftime('%Y-%m-%d')
    date_from = (datetime.now() - timedelta(days=30)).strftime('%Y-%m-%d')

    # Stap 1-4: standaard matching
    df_crm      = fetch_crm_data(date_from, date_to)
    df_cbs      = fetch_cbs_data()
    crm_profiel = build_crm_profile(df_crm, top_postcode='4001')
    df_matches  = run_matching_engine(crm_profiel, df_cbs)
    ...
    print(f"✅ Matches klaar. {len(df_matches[df_matches['score'] >= 70])} tweelingwijken gevonden.")

    # Stap 5: Kansen kaart — elke zondag volledig herberekenen
    # (CBS data verandert niet dagelijks, wekelijks is genoeg)
    dag_van_week = datetime.now().weekday()  # 6 = zondag
    if dag_van_week == 6:
        print("\n📊 Zondag — kansen kaart herberekenen...")
        try:
            from kansen_generator import genereer_kansen

            # Bouw profiel uit echte kassadata
            profiel = {
                'inkomen':   int(crm_profiel.get('inkomen', 35000)),
                'leeftijd':  int(crm_profiel.get('leeftijd', 35)),
                'pct_vrouw': float(crm_profiel.get('pct_vrouw', 68)),
                'pct_hbo':   float(crm_profiel.get('pct_hbo', 42)),
                'pct_koop':  float(crm_profiel.get('pct_koop', 55)),
            }
            genereer_kansen(profiel=profiel, upload=True)
            print("✅ Kansen kaart bijgewerkt in Supabase")
        except Exception as e:
            print(f"⚠️  Kansen generator mislukt (non-fatal): {e}")
    else:
        print(f"⏭️  Kansen kaart overgeslagen (alleen zondag — vandaag is dag {dag_van_week})")
